...
test-iosxe-cli-to-acton-adata:
    ...
    RESULT=$(curl -s -X POST http://localhost:8080/api/v1/convert \
      -H "Content-Type: application/json" \
      -d '{
        "input": "interface GigabitEthernet1\n description \"IOS XE test interface\"\n ip address 10.1.1.1 255.255.255.0\n no shutdown",
        "format": "cli",
        "target_format": "acton-adata",
        "platform": "iosxe 17.9.1-local"
      }')
    echo "$RESULT" | jq .
...
test-iosxe-netconf-to-cli:
    ...
    RESULT=$(curl -s -X POST http://localhost:8080/api/v1/convert \
      -H "Content-Type: application/json" \
      -d '{
        "input": "<native xmlns=\"http://cisco.com/ns/yang/Cisco-IOS-XE-native\"><interface><GigabitEthernet><name>3</name><description>IOS XE NETCONF test</description></GigabitEthernet></interface></native>",
        "format": "netconf",
        "target_format": "cli",
        "platform": "iosxe 17.9.1-local"
      }')
...
test-iosxe-cli-to-json:
    ...
    RESULT=$(curl -s -X POST http://localhost:8080/api/v1/convert \
      -H "Content-Type: application/json" \
      -d '{
        "input": "interface GigabitEthernet5\n description \"IOS XE to JSON\"\n ip address 10.5.5.1 255.255.255.0\n no shutdown",
        "format": "cli",
        "target_format": "json",
        "platform": "iosxe 17.9.1-local"
      }')
